...
mkt_cap = read.csv("data/mkt_cap.csv")

colnames(mkt_cap)[1] = "Country.Name"
mkt_cap$`Country.Name` = tolower(mkt_cap$`Country.Name`)
mkt_cap$`Country.Name` = str_replace_all(mkt_cap$`Country.Name`," ",".")
mkt_cap[mkt_cap$`Country.Name` == "cote.d'ivoire",]$`Country.Name` = "côte.d.ivoire"
mkt_cap[mkt_cap$`Country.Name` == "egypt,.arab.rep.",]$`Country.Name` = "egypt"
mkt_cap[mkt_cap$`Country.Name` == "hong.kong.sar,.china",]$`Country.Name` = "hong.kong"
mkt_cap[mkt_cap$`Country.Name` == "korea,.rep.",]$`Country.Name` = "south.korea"
mkt_cap[mkt_cap$`Country.Name` == "north.macedonia",]$`Country.Name` = "macedonia"
mkt_cap[mkt_cap$`Country.Name` == "russian.federation",]$`Country.Name` = "russia"
mkt_cap[mkt_cap$`Country.Name` == "slovak.republic",]$`Country.Name` = "slovakia"
mkt_cap[mkt_cap$`Country.Name` == "united.states",]$`Country.Name` = "us"
mkt_cap[mkt_cap$`Country.Name` == "united.kingdom",]$`Country.Name` = "uk"
mkt_cap[mkt_cap$`Country.Name` == "venezuela,.rb",]$`Country.Name` = "venezuela"

temp_countries = unique(substr(countries,4,1000))
# setdiff(temp_countries, mkt_cap[mkt_cap$Country.Name %in% temp_countries,]$Country.Name)
mkt_cap$Country.Name = paste0("eq_",mkt_cap$Country.Name)
mkt_cap = mkt_cap[mkt_cap$Country.Name %in% countries,]
# rm(temp_countries)
colnames(mkt_cap)[-1] = substr(colnames(mkt_cap)[-1],2, 1000)
mkt_cap = mkt_cap[order(mkt_cap$Country.Name),]
mkt_cap = mkt_cap[,colSums(is.na(mkt_cap))<nrow(mkt_cap)]
mkt_cap = mkt_cap[,c(1,13:46)]
sums = colSums(mkt_cap[,-1], na.rm=TRUE)

# wavg = function(group, avg_name, weight_name)
# {
...
weighted_rsq = matrix(ncol = 136, nrow = nrow(mkt_cap))

for(i in 1:136)
{
  for(j in (1:length(mkt_cap$Country.Name)))
  {
    weighted_rsq[j,i] = (1- rsq[which(countries == mkt_cap$Country.Name[j]), i]) * mkt_cap[mkt_cap$Country.Name == mkt_cap$Country.Name[j],as.character(year(quartersList[i]))]/sum(mkt_cap[,as.character(year(quartersList[i]))], na.rm = TRUE)

  }
}
# mkt_cap[mkt_cap$Country.Name == "argentina",as.character(year(quartersList[i]))]/sum(mkt_cap[,as.character(year(quartersList[i]))], na.rm = TRUE)
# eq = (colSums(weighted_rsq, na.rm = TRUE))*100
...
{
  a = NW(lm((rowMeans(100 - rsq, na.rm = TRUE)) ~ ., data = factors[-1]))
...
liquidity = read.csv("data/stock illiquidity.csv")
...
construct_table9_data = function(grep_search)
{
  ...
  equity[,c("Country","Div", "year", "TED",  "VIX",  "SENT", "FEDFUNDS","INTERNET","ERM",  "EUROZONE", "COVID", "Aggregate.Economic.Risk", "Aggregate.Financial.Risk", "Aggregate.Poliitical.Risk","liquidity", "PC","insti_loan" )]
}
```
...
```{r echo=FALSE, warning=FALSE}
write.csv(round(cor(subset(construct_table9_data("^eq_"), select = c(TED, VIX, SENT, FEDFUNDS, INTERNET, ERM, EUROZONE, COVID,liquidity, PC,Aggregate.Economic.Risk,Aggregate.Financial.Risk,Aggregate.Poliitical.Risk))),4),"temp.csv")
```
