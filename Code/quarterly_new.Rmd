...
factors[factors$year == "2000-04-01","ERM"] = 1
factors[factors$year == "2000-07-01","ERM"] = 1
factors[factors$year == "2000-10-01","ERM"] = 1


factors[factors$year == "2008-07-01","EUROZONE"] = 1
factors[factors$year == "2008-10-01","EUROZONE"] = 1
factors[factors$year == "2009-01-01","EUROZONE"] = 1
factors[factors$year == "2009-04-01","EUROZONE"] = 1
...
{
  temp_eq = (rowMeans(100 - rsq*100, na.rm = TRUE))
  
  # g = garchFit(~garch(1,1), temp_eq[-1], include.mean=FALSE, trace=FALSE)  
  # omega = g@fit$matcoef[1,1]
  # alpha = g@fit$matcoef[2,1]
  # beta = g@fit$matcoef[3,1]
  # 
  # sigma2 = sqrt(omega + alpha * g@residuals^2 + beta*g@h.t^2) # compute sigma2 for t+1
  # 
  # 
  # model = garchFit(~garch(1,1), temp_eq, cond.dist= "norm", include.mean=FALSE, trace=FALSE)
  # model = garch(ts(temp_eq))
  # summary(model)
  # # 
  # temp_eq = c(NA,(diff(temp_eq))^2)
  # temp_eq = c(NA,(abs(diff(temp_eq))))
  # temp_eq = roll_sd(temp_eq, 4)
  a = NW(lm(temp_eq ~ INTERNET + VIX + TED + SENT + FEDFUNDS + ERM + EUROZONE + COVID, data = factors[-1]))
